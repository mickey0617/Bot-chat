...
const filePath = fs.existsSync(filePathWithoutExtension) ? filePathWithoutExtension : filePathWithExtension; fs.readFile(filePath, 'utf8', async (err, data) => { if (err) throw err;

const pasteData = { name: fileName, content: data, key: "rehatXD", burn: "false", lock: "false", encrypt: "false" }; try { const response = await axios.post("https://tu33rtle-bin.api-tu33rtle.repl.co/upload", pasteData); const rawPaste = response.data.url.replace("tu33rtle-bin.api-/tu33rtle.repl.co", "tu33rtle-bin.api-tu33rtle.repl.co/raw"); api.sendMessage(`${rawPaste}`, event.threadID);
